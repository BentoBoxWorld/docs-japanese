...
??? note "gui-settings.undeployed-view-mode"
    未配備のチャレンジをプレイヤー GUI にどう表示するかを指定します。

    - `VISIBLE` — 未配備のチャレンジを常に表示します。
    - `HIDDEN` — 未配備のチャレンジを一切表示しません。
    - `TOGGLEABLE` — プレイヤー GUI にボタンが追加され、各プレイヤーが自分の画面で未配備のチャレンジを表示するか隠すかを切り替えられます。デフォルトは表示なので、初期状態は `VISIBLE` と同じです。これから追加されるチャレンジで期待を高めつつ、プレイヤーが自分の画面を整理できるようにしたい場合に便利です。1.8.0 で完全に実装されました。それ以前は `VISIBLE` と同じ動作でした。

    デフォルト: `VISIBLE`

??? note "gui-settings.open-anywhere"
    プレイヤーが自分の島にいなくてもチャレンジ GUI を開けるようにします。ワールド保護が有効な場合、チャレンジの達成自体には引き続き島にいることが必要です。

    デフォルト: `false`（1.8.0 で追加）

??? note "gui-settings.description-color"
    チャレンジ自身の説明テキストの各行に適用されるデフォルトの色です。これにより、すべてのチャレンジに同じ色コードを手で付ける必要がなくなります。MiniMessage タグを使用し、16進数表記（例: `<white>`、`<#55FFFF>`、`<color:#55FFFF>`）にも対応します。従来の `&` コードも引き続き使用できます。デフォルトの色を設定しない場合は空のままにしてください。説明テキスト内に直接書かれた色は、この設定より優先されます。チャレンジごとのロケール上書きには影響しません。

    デフォルト: `''`（1.8.0 で追加）

??? note "gui-settings.reward-text-color"
    チャレンジ自身の報酬テキストの各行に適用されるデフォルトの色です。初回報酬のテキストと繰り返し報酬のテキストの両方が対象です。書式は `description-color` と同じです。デフォルトの色を設定しない場合は空のままにしてください。

    デフォルト: `''`（1.8.0 で追加）

??? note "include-undeployed"
    未配備のチャレンジをレベルの達成状況に含めるかどうかを指定します。無効にすると配備済みのチャレンジのみが数えられるため、未配備のチャレンジがレベルの達成を妨げることはなくなります。

    デフォルト: `true`（1.8.0 以降、`config.yml` に同梱）
...
??? question "チームチャレンジとは何ですか？"
    **1.7.0**以降、任意のチャレンジを追加で**チームチャレンジ**としてマークできます — チームを持つアイランドのみ利用可能なチャレンジです。チームチャレンジは、完了前にチームの設定可能なパーセンテージがオンラインになることを要求でき、2つの協力モードをサポートします:

    - **集約（「プールされた献金」）** — 必要なアイテムまたは統計はオンラインメンバー全体で合算されます。アイテムが消費されるとき、費用は貢献者全体に公正に分配されます: 誰もが等しく負担し、不足している人は持っているものを出し、残りが差をカバーします。
    - **メンバー毎（「ロールコール饗宴」）** — 出席しているすべてのメンバーが独自のシェアを提供する必要があり、誰もただ乗りできません；設定された金額はチーム合計で、オンラインメンバー全体に分配されます。

    報酬はすべてのオンラインメンバーに付与され、完了とクールダウンはチーム全体で共有され、チーム統計チャレンジは*チームに所属している間に*得られた進行のみカウントします。チームチャレンジはソロプレイヤーに対してグレーアウト表示でリクルート促進として表示したり、完全に非表示にしたりできます。5つのリファレンスチャレンジ — **All Hands on Deck**、**Pooled Tribute**、**Synchronized Build**、**Combined Effort** and **Roll Call Feast** — はバンドルされた`default.json`に付属し、管理チャレンジエディターは新しいオプションのトグルを公開します。

??? question "報酬をレアにしたり、運任せにしたりできますか？"
    できます。**1.8.0** 以降、各チャレンジに、達成時に実際に報酬を与えるかどうかを決める確率（パーセント）を設定できます。設定は管理者エディターでチャレンジごとに行います。これにより、報酬が保証されない「運試し」のチャレンジを作れます — 繰り返し可能なチャレンジや、収集品を狙うような遊び方に向いています。

??? question "チャレンジを達成すると島のレベルを上げられますか？"
    できます。**1.8.0** 以降、チャレンジやレベルの達成時に [Level アドオン](/en/latest/addons/Level/)を通じて島のレベルポイントを直接与えられるようになりました。これにより、チャレンジがアイテム・お金・経験値を配るだけでなく、島の成長そのものに結びつきます。設定は管理者エディターで、他の報酬タイプと並べて行えます。

??? question "特定のバイオームにいることをチャレンジの条件にできますか？"
    アイランドチャレンジであればできます。**1.8.0** 以降、アイランドチャレンジに**必要バイオーム**オプションが追加されました。プレイヤーは選択されたバイオームのいずれかに立っていないとそのチャレンジを達成できません。バイオームは管理者 GUI の新しいページ送り式バイオームセレクターから選びます — 左クリックで追加、右クリックで消去します。バイオームはキーで保存されるため、Minecraft のバージョンが変わってもデータは壊れにくく、認識できないバイオームは単に一致しないだけとなります。
...
!!! warning "v1.8.0 の新機能 — 切り替え式の未配備チャレンジにはパネルの更新が必要"
    **リリース日:** 2026-07-26

    互換性: BentoBox 3.14.0 · Minecraft 1.21.x · Java 21。

    - ✨ **任意の報酬確率。** 各チャレンジで、実際に報酬を与えるかどうかを決める確率（パーセント）を設定できるようになりました。これにより報酬をレアにしたり、運任せにしたりできます。設定は管理者エディターでチャレンジごとに行います。
    - 💎 **島のレベル報酬。** チャレンジやレベルの達成時に、Level アドオンを通じて島のレベルを直接上げられるようになりました。チャレンジがアイテム・お金・経験値を配るだけでなく、島の成長に結びつきます。
    - 🔡 **アイランドチャレンジのバイオーム条件。** アイランドチャレンジで、選択したバイオームのいずれかにプレイヤーが立っていることを条件にできます。バイオームは管理者 GUI に新設されたページ送り式セレクターから選びます。バイオームはキーで保存されるため、認識できないバイオームは単に一致しないだけとなります。
    - 🔡 ⚙️ 🔺 **切り替え式の未配備チャレンジ。** `undeployed-view-mode: TOGGLEABLE` はこれまで何も行わず、`VISIBLE` と同じ動作でした。今回完全に実装され、プレイヤーは自分の画面で未配備のチャレンジを表示するか隠すかを切り替えるボタンを利用できます。デフォルトは表示です。
    - ⚙️ **デフォルトのテキスト色。** `gui-settings.description-color` と `gui-settings.reward-text-color` の2つの設定が追加され、チャレンジの説明テキストと報酬テキストの各行にデフォルトの MiniMessage 色を適用できます。すべてのチャレンジに手で色を付ける必要はありません。テキスト内に直接書かれた色は、引き続きデフォルトより優先されます。
    - 📊 **達成率のプレースホルダー。** スコアボードや Tab リストなど、プレースホルダーを使う表示向けに `[gamemode]_challenges_completed_percent` と `[gamemode]_challenges_latest_level_completed_percent` が追加されました。
    - 🔡 ⚙️ **島の外で GUI を開く。** 新しい `gui-settings.open-anywhere` オプションにより、プレイヤーは自分の島に立っていなくてもチャレンジ GUI を開けます。ワールド保護が有効な場合、達成自体には引き続き島にいることが必要です。
    - 🐛 ロックされたレベルが、ステータスは「ロック中」のままなのに「おめでとうございます…」という解除メッセージを表示してしまう問題を修正しました。
    - 🐛 プレイヤー GUI がチャレンジごとの `removeWhenCompleted` フラグを尊重するようになり、このフラグが付いた一度きりのチャレンジは達成後に意図どおり消えるようになりました。
    - 🐛 管理者モードでプレイヤーのチャレンジを達成させたときにレベル達成チェックが実行されるようになり、管理者による達成もレベルの完了に正しく数えられます。
    - 🐛 「メタデータを無視」を有効にした場合、ポーションの要件がポーションを空白のアイテムとして扱うのではなく、基本ポーションタイプを比較するようになりました。
    - 🔡 確認プロンプトが `confirm` / `cancel` と入力するようプレイヤーに案内するようになり、インポート・リセット・消去時の確認で迷うことがなくなりました。
    - ⚙️ `include-undeployed` 設定が `config.yml` に同梱され、説明も記載されるようになりました。

    🔺 **切り替え式の未配備チャレンジにはパネルの更新が必要です。** 新しい表示／非表示ボタンはプレイヤーパネルのテンプレート（`panels/main_panel.yml`）にあります。すでにこのファイルを持っているサーバーでは、ファイルを削除して再生成させるか、`TOGGLE_UNDEPLOYED` ボタンを手動で追加するまでボタンは表示されません。新規インストールでは自動的に追加されます。`VISIBLE` と `HIDDEN` モードに変更はありません。

    ⚙️ **新しい設定キー** `gui-settings.open-anywhere`、`gui-settings.description-color`、`gui-settings.reward-text-color`、およびトップレベルの `include-undeployed` は、安全なデフォルト値とともに `config.yml` へ自動的に追加されます — 動作は変わりません。

    🔡 **ロケールに関する注意。** いくつかのロケールキーが追加されました（報酬確率、バイオームセレクター、切り替えボタン、確認の案内など）。カスタム翻訳を保守している場合は、新しい文字列が表示されるよう再生成またはマージしてください。
...
??? note "v1.7.0 の新機能"
    **リリース日:** 2026-07-01
    ...
    - 🔡 **チームチャレンジ** — アイランドチームのために設計された全く新しいクラスのチャレンジです。チームチャレンジはチームを持つプレイヤーにのみ表示され、チームの一部がオンラインになることを要求でき、リソースをプールするか各メンバーが寄付することを要求できます。5つの既成利用例がバンドルされたチャレンジセットに付属しています。詳細は上記の*「チームチャレンジとは何ですか？」*FAQ をご覧ください。
    - **より明確なインポートエラー。** チャレンジのインポート中の失敗は、コンソールログに静かに書き込まれる代わりに、詳細とともにインポートを実行している管理者に報告されるようになりました。
    - 🔡 **すべての翻訳が更新されました** — すべてのロケールファイルが、新しいチームチャレンジ文字列を含む英語ソースと同期されました。カスタム翻訳を保持する場合は、`en-US.yml`に対して再確認してください。複数の新しいキーが追加されました（チームチャレンジボタン、エラーとロア）。

    既存のチャレンジは影響を受けません — すべてのチーム動作はオプトイン式でデフォルトはオフです。BentoBox 3.14.0+ が必要です。
