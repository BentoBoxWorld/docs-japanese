...
??? note "v2.3.2 の新機能"
    **リリース日：** 2026-06-15
    ...
    2.3.1 の小規模なフォローアップリリースで、海洋バイオーム修正を完成させ、アドオンの設定書き込み方法を修正します。

    - ⚙️ **`change-ocean-biomes` が `config.yml` に追加されました。** このオプション（2.3.0 で追加）はテンプレート設定に書き出されていなかったため、ディスク上に現れず、設定できませんでした。
    - 🔧 **`config.yml` がロード時に自己修復するようになりました。** ロード後に設定が書き戻されるため、新バージョンで追加されたオプションは既存の設定に自動的にデフォルト値で追加されます — これがオプションが見えなかった根本原因でした。

    🔺 **2.3.0 / 2.3.1 からのアップグレード — 設定編集は不要です。** 初回ロード時に設定が自己修復し、`change-ocean-biomes` は自動的に `true` として書き出され、バイオーム変更がそのまま機能します（意図的に `false` に設定して海洋を保護していた場合、その値は保持されます）。

    [Release v2.3.2](https://github.com/BentoBoxWorld/Biomes/releases/tag/2.3.2)

??? warning "v2.3.1 の新機能 — 海洋アイランドの修正"
    **リリース日：** 2026-06-14

    - ⚙️🔺 **バイオーム変更が海洋アイランドにも再び適用されるようになりました。** `change-ocean-biomes` オプション（2.3.0 で追加）のデフォルトが `false` だったため、Biomes はすでに海洋バイオームであるすべてのブロックをスキップしていました — そのため、全体が海洋バイオーム（SkyBlock/AcidIsland のボイドワールドは全体が `warm_ocean`）のアイランドは、成功と報告されながら完全にスキップされていました。**デフォルトは `true` になりました。** [#171](https://github.com/BentoBoxWorld/Biomes/issues/171) を修正。

    🔺 **注意：** 2.3.0 からアップグレードするサーバーでは、このバージョンでは既存の `config.yml` に `change-ocean-biomes` の行が自動的には書き込まれませんが、新しいデフォルト（`true`）はメモリ上で有効になるため、バイオーム変更は再び機能します。この設定の可視性の問題は 2.3.2 で完全に修正されています。
