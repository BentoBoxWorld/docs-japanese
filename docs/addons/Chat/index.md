...
??? note "v1.4.0 の新機能"
    **リリース日：** 2026-04-13
    ...
    - **追加ワールドでのチームチャット** — チームチャットがゲームモードワールド以外でも機能するようになりました。`config.yml` の `team-chat.extra-chat-worlds` で、ゲームモードごとにチームチャットを捕捉すべき追加ワールド（スポーン、ハブなど）を指定できます。
    - **チームチャットのミュート** — プレイヤーはチームを離脱せずに `/is muteteamchat` で受信するチームチャットを無効化できるようになりました。ミュートはチームの離脱/キック時に自動的に解除されます。
    - **MiniMessage への移行** — 23 個のロケールファイルすべてが、従来の `&` カラーコードから MiniMessage に変換されました。カスタマイズされたロケールファイルがある場合は、`&a` → `<green>` のように更新するか、削除して再生成させてください。
    - バグ修正：アイランドを持たないプレイヤーが `/is teamchat` を使用した際の null pointer exception を修正。
