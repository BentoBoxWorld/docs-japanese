...
??? note "v1.1.1 の新機能"
    **リリース日:** 2026-06-28
    ...
    - 🔺 🐛 **平和難易度でのワールド作成中のサーバークラッシュを修正（Paper 26.2）。** ネザーポピュレーターはモンスター（溺死者/ガーディアン/シニアガーディアン）をスポーンしようとしました。これは平和難易度では許可されず、チャンクシステムをクラッシュさせました。モンスタースポーンは平和難易度ではスキップされるようになり、ワールド生成がクラッシュしないように防御的に保護されます。
