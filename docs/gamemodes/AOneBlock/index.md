...
プレイヤーは魔法のような1つのブロックの上でサバイバルしなければなりません...

作成・メンテナンス: [tastybento](https://github.com/tastybento)

{{ addon_description("AOneBlock") }}

OneBlockは宇宙に浮かぶ1つのブロックの上にあなたを置きます。ブロックは1つしかありません。次はどうしますか？
...
=== "fixedBlocks"
    !!! summary "説明"
        fixedBlocksセクションでは、プレイヤーがブロックを破壊した際に特定のブロックを強制的に出現させることができます。最初はフェーズ内のブロック番号で、次にBukkit Materialが続きます。フェーズの最初のブロックのインデックスは0であり、フェーズの実行時間より大きい数を追加すると、そこには到達しないことになります。
